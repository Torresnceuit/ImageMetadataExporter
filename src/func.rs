...
pub fn parsing_metadata(exif: Exif) -> Value {
    ...
    let created_time = match exif.get_field(Tag::DateTimeOriginal, In::PRIMARY) {
        Some(created_time) => Some(created_time.display_value().with_unit(&exif).to_string()),
        None => None,
    };

    let modified_time = match exif.get_field(Tag::DateTimeDigitized, In::PRIMARY) {
    ...
    let capture_time = match exif.get_field(Tag::DateTime, In::PRIMARY) {
